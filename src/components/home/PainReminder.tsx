  ...
  return (
    <section className="relative overflow-hidden bg-ivory py-16 sm:py-24">
      ...
      <div className="relative mx-auto max-w-2xl px-5 text-center sm:px-6">
        <h2 className="font-serif text-[2rem] leading-[1.1] text-ink sm:text-4xl">
        ...
        <div className="mt-7 flex flex-wrap items-center justify-center gap-2.5 sm:mt-8 sm:gap-3">
          {QUESTIONS.map((q) => (
            <span
              key={q}
              className="rounded-full border border-champagne bg-white/70 px-4 py-2.5 text-[13px] text-ink-soft backdrop-blur-sm"
            >
              {q}
            </span>
          ))}
